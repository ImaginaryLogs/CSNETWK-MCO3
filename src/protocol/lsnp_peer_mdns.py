...
verbose_mds_logger = logger.get_logger('[green][ mDNS ][/] |:')
...
class LSNPPeer:
    ...
    def _register_mdns(self):
        ...
        if self.verbose:
            verbose_mds_logger.info(f"[mDNS] Registered: {info.name}")
    ...
    def _start_threads(self):
        ...
        if self.verbose:
            verbose_mds_logger.info("[mDNS] Discovery started")
    ...
    def _listen(self):
        while True:
            ...
            try:
                raw = data.decode()
                if self.verbose:
                    verbose_mds_logger.info(f"[RECV] From {addr}: \n{raw[:100]}{'...' if len(raw) > 100 else ''}")
                ...
                if self.verbose:
                    verbose_mds_logger.info(f"[ERROR] Malformed message from {addr}: {e}")
    ...
    def _handle_kv_message(self, kv: dict, addr):
        ...
        if msg_type == "PROFILE":
            ...
            if from_id not in self.peer_map:
                ...
                if self.verbose:
                    verbose_mds_logger.info(f"[PROFILE] {display_name} ({from_id}) joined from {ip}")
            ...
            if to_id != self.full_user_id:
                if self.verbose:
                    verbose_mds_logger.info(f"[DM IGNORED] Not for us: {to_id}")
                return
            
            if not validate_token(token, "chat"):
                if self.verbose:
                    verbose_mds_logger.info(f"[DM REJECTED] Invalid token from {from_id}")
                return
            ...
            if message_id in self.ack_events:
                ...
                if self.verbose:
                    verbose_mds_logger.info(f"[ACK] Received for message {message_id}")
        
        elif msg_type == "PING":
            user_id = kv.get("USER_ID", "")
            if self.verbose:
                verbose_mds_logger.info(f"[PING] From {user_id}")
    ...
    def _handle_json_message(self, msg: dict, addr):
        ...
        if msg_type == "DM":
            ...
            verbose_mds_logger.info(f"{sender_id}: {content}")
    ...
    def _send_ack(self, message_id: str, addr):
        ...
        if self.verbose:
            verbose_mds_logger.info(f"[ACK SENT] For message {message_id} to {addr}")
    ...
    def _on_peer_discovered(self, peer: Peer):
        if self.verbose:
            verbose_mds_logger.info(f"[DISCOVERED] {peer.display_name} ({peer.user_id})")
    ...
    def send_dm(self, recipient_id: str, content: str):
        ...
        for attempt in range(RETRY_COUNT):
            ...
            if self.verbose:
                verbose_mds_logger.info(f"[DM SEND] Attempt {attempt + 1} to {recipient_id}")
            ...
            if self.verbose:
                verbose_mds_logger.info(f"[RETRY] {attempt + 1} for {recipient_id}")
    ...
    def broadcast_profile(self):
        ...
        for peer in self.peer_map.values():
            self.socket.sendto(msg.encode(), (peer.ip, peer.port))
        if self.verbose:
            verbose_mds_logger.info("[BROADCAST] Profile message sent.")
    ...
    def send_ping(self):
        ...
        if self.verbose:
            verbose_mds_logger.info(f"[PING] Sent to {broadcast_addr}")
